...
WORKLOAD_UTIL="${WORKLOAD_UTIL:-45-90}"
...
MODEL_TABLE="$(read_model_table "$MODELS_FILE" || true)"
if [ -z "$MODEL_TABLE" ]; then
  MODEL_TABLE=$'gpu-sim\tgpu-sim\t0\t'
fi
export MODEL_TABLE
...
for node in sorted(inv['nodes'], key=lambda item: item['name']):
    for gpu_index in range(int(node['gpuCount'])):
        release, model = models[i % len(models)]
        print(node['name'], node['pool'], gpu_index, release, model)
        i += 1
")
...
while read -r node pool gpu_index release model; do
